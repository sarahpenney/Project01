$(function() {
    ...
    $character.animate(pos, charSpeed, function() {
      ...
      //   $('#encounterTalk').show();
        if(checkIfCloseEncounter()) {
          ...
          var id = parseFloat($encounter.attr('id').replace('encounter', ''));
          var $nextEncounter = $encounters.filter('#encounter' + (id+1));

          $encounter.removeClass('active');
          $nextEncounter.addClass('active');
        }
      }
    );
  }
  ...
  function checkIfCloseEncounter() {
    ...
    var $encounter = $encounters.filter('.active');
    var encounterPos = $encounter.position();

    encounterPos.bottom = encounterPos.top + $encounter.height();
    encounterPos.right = encounterPos.left + $encounter.width();
